...
def train(model, opt, shdlr, train_batches, cur_idx, num_batch, args):
    ...
    for batch in train_batches[cur_idx:cur_idx+num_batch]:
        parsed_batch, item_indices, nbr_indices = parse_sampled_batch(batch[0])
        opt.zero_grad()
        shdlr.step()
        ...
        loss += args.alpha * F.binary_cross_entropy_with_logits(
            nbr_logits[nbr_indices], torch.ones_like(nbr_logits)[nbr_indices])
...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='social reg term weight')
