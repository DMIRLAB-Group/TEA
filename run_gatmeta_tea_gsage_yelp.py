...
#     def __init__(self, in_dim, out_dim, head, is_out=False, is_bais=True):
...
#         if is_bais is True:
#             if is_out is False:
#                 self.bais = torch.nn.Parameter(torch.Tensor(head * out_dim))
#             else:
#                 self.bais = torch.nn.Parameter(torch.Tensor(out_dim))
#             torch.nn.init.zeros_(self.bais)
#             # self.bais = torch.nn.Parameter(torch.Tensor(out_dim))
#
#         self.W = torch.nn.Linear(in_dim, head * out_dim)
#         self.A = torch.nn.Parameter(torch.Tensor(1, head, 2 * out_dim))
#         torch.nn.init.xavier_uniform_(self.W.weight, gain=1.414)
#         torch.nn.init.xavier_uniform_(self.A, gain=1.414)
#
#     def forward(self, x, edge_index):
#         x = self.W(x).view(-1, self.head, self.out_dim)
#         out = self.propagate(edge_index, x=x, node_num=x.shape[0])
#
#         if self.is_out:
#             out = out.mean(dim=1)
#         else:
#             out = out.view(-1, self.head * self.out_dim)
#
#         if self.is_bais:
#             out += self.bais
#         return out
#
#     def message(self, index, x_i, x_j, node_num):
#         e = (torch.cat((x_i, x_j), dim=-1) * self.A).sum(dim=-1)
#         e = F.leaky_relu(e)
#         e = softmax(e, index, num_nodes=node_num)
#         return x_j * e.unsqueeze(-1)
#
#
# class GAT(torch.nn.Module):
#     def __init__(self, in_dim, hidden_dim, out_dim, head, layer_num, self_loop=True):
#         super(GAT, self).__init__()
#         self.self_loop = self_loop
#
#         self.lins = torch.nn.ModuleList()
#         self.lins.append(GATlayer(in_dim, hidden_dim, head))
#         for _ in range(layer_num - 2):
#             self.lins.append(GATlayer(head * hidden_dim, hidden_dim, head))
#         self.lins.append(GATlayer(head * hidden_dim, out_dim, head, True))
#
#     def forward(self, x, edge_index):
#         if self.self_loop:
...
class CRFGNN(nn.Module):
    def __init__(self,
        ...
        self.meta_rnn = nn.GRU(input_size=edim//8, hidden_size=edim // 8, num_layers=1, batch_first=True)
        # self.meta_gat = GAT(edim, edim, edim, 1, args.meta_gat_layer_num)
        self.meta_gat_W = torch.nn.Linear(edim, edim//8)
        self.meta_gat_A = torch.nn.Parameter(torch.Tensor(1, 2 * edim//8))
        self.meta_gat_item = nn.Linear(edim, edim//8)
    ...
    def get_GAT(self, uid, all_nbr):
        u_nbr = all_nbr[uid]  # B*nbr B*sl*meta*nbr
        u_nbr = torch.cat((u_nbr, uid.unsqueeze(-1)),-1) #B*(nbr+1)  B*sl*meta*(nbr+1)
        u_nbr_mask = torch.BoolTensor(u_nbr != 0).to(self.dev).unsqueeze(-1)
        uid = uid.to(self.dev)
        u_nbr = u_nbr.to(self.dev)

        uid_emb = self.meta_gat_W(self.user_embs(uid))  # B*d B*sl*meta*d
        u_nbr_emb = self.meta_gat_W(self.user_embs(u_nbr)) * u_nbr_mask    # B*(nbr+1)*d  B*sl*meta*(nbr+1)*d
        u_cat = torch.cat((uid_emb.unsqueeze(-2).expand_as(u_nbr_emb), u_nbr_emb), -1) #B*(nbr+1)*(2*dim)  B*sl*meta*(nbr+1)*(2*dim)
        # print("u_cat",u_cat.shape)
        u_att = (u_cat * self.meta_gat_A).sum(dim=-1) #B*(nbr+1)  B*sl*meta*(nbr+1)
        u_att = F.leaky_relu((u_att))
        u_att = u_att.softmax(dim=-1).unsqueeze(-1)#B*(nbr+1)*1     B*sl*meta*(nbr+1)*1

        gat_uid_emb = self.dropout(u_nbr_emb * u_att).sum(-2) #B*d  B*sl*meta*d
    ...
    # def metaGAT(self, uid, meta, all_nbr):
    #
    #     # u_nbr = all_nbr[uid].to(self.dev) #B* nbr
    #     # u_nbr_mask = torch.BoolTensor(u_nbr != 0).to(self.dev).unsqueeze(-1)
    #     # meta_nbr = all_nbr[meta].to(self.dev)#B*sl*meta*nbr
    #     # meta_nbr_mask = torch.BoolTensor(meta_nbr != 0).to(self.dev).unsqueeze(-1)
    #     #
    #     # uid_emb = self.meta_gat_W(self.user_embs(uid))  # B*d
    #     # meta_emb = self.meta_gat_W(self.user_embs(meta)) #B*sl*meta_maxlen
    #     # u_nbr_emb = self.meta_gat_W(self.user_embs(u_nbr)) * u_nbr_mask #B*nbr*d
    #     # meta_nbr_emb = self.meta_gat_W(self.user_embs(meta_nbr)) * meta_nbr_mask#B*sl*meta_maxlen*nbr*d
    #     #
    #     # u_cat = torch.cat((uid_emb.unsqueeze(-2).expand_as(u_nbr_emb), u_nbr_emb), -2)
    #     # meta_cat = torch.cat((meta_emb.unsqueeze(-2).expand_as(meta_nbr_emb), meta_nbr_emb), -2)
    #     #
    #     # u_att = (u_cat * self.meta_gat_A).sum(dim=-1)
    #     # meta_att = (meta_cat * self.meta_gat_A).sum(dim=-1)
    #
    #     uid_emb = self.get_GAT(uid, all_nbr)
    #     meta_emb = self.get_GAT(meta, all_nbr)





    def meta2feat(self, uid, seq, meta, all_nbr):
        #meta B * sl * meta_maxlen
        seq_len = seq.shape[-1]
        user_num = uid.shape[0]
        meta_maxlen = meta.shape[-1]

        # gat_user_emb = self.meta_gat(all_user_emb, edge_index)  # all_user_num * dim
        ...
        # user_emb = gat_user_emb[uid].unsqueeze(-2).unsqueeze(-2)#B*1*1*dim
        user_emb = self.get_GAT(uid, all_nbr).unsqueeze(-2).unsqueeze(-2)#B*1*1*dim
        # print('user_emb',user_emb.shape)
        seq_emb = self.dropout(self.item_embs(seq.to(self.dev)))  # B*sl*1*dim
        seq_emb = self.meta_gat_item(seq_emb)
        # print('seq_emb',seq_emb.shape)
        # meta_emb = self.dropout(self.user_embs(meta.to(self.dev))).unsqueeze(-2)#B * sl * meta_maxlen * 1 * dim
        # meta_emb = gat_user_emb[meta].unsqueeze(-2)#B * sl * meta_maxlen * 1 * dim
        meta_emb = self.get_GAT(meta, all_nbr).unsqueeze(-2)#B * sl * meta_maxlen * 1 * dim
        # print('meta_emb',meta_emb.shape)
        # print("emb user",user_emb.shape,'seq',seq_emb.shape,'meta',meta_emb.shape)
        meta_path_emb = torch.cat((seq_emb, user_emb.expand_as(seq_emb)), dim=-2).unsqueeze(-3).expand(-1, -1,
                                                                                                       meta_maxlen, -1,
                                                                                                       -1)  # B*sl*1*2*dim
        # print("first meta_path_emb",meta_path_emb.shape)
        ...
        meta_path_emb = meta_path_emb.view(-1, 3, self.edim//8)
        # print("meta_path_emb",meta_path_emb.shape)
    ...
    def dual_forward(self, batch, all_nbr):
        uid, seq, pos, neg, nbr, nbr_iid, meta = batch

        meta_user_emb, meta_item_emb = self.meta2feat(uid, seq, meta, all_nbr)
    ...
    def eval_all_users(self, eval_loader, all_nbr):
        all_scores = list()
        self.eval()
        with torch.no_grad():
...
def evaluate(model, eval_loader, eval_users, all_nbr):
    model.eval()
    hr5 = hr10 = hr20 = ndcg5 = ndcg10 = ndcg20 = 0.0
    all_scores = model.eval_all_users(eval_loader, all_nbr)
...
def train(model, opt, shdlr, train_loader, num_item, all_nbr, args):
    model.train()
    total_loss = 0.0
    for batch in train_loader:
        parsed_batch, indices = parse_sampled_batch(batch)
        opt.zero_grad()
        pos_logits, neg_logits, user_emb, pos_hi, neg_hi = model.dual_forward(parsed_batch, all_nbr)
...
def load_ds(args, item_num):
    ...
    # if os.path.exists('./datasets/Yelp/edge_index.npy'):
    #     #     edge_index = np.load('./datasets/Yelp/edge_index.npy')
    ...
    #     #     edge_index = [[],[]]
    #     #     for u1 in u2u_dict:
    #     #         for u2 in u2u_dict[u1]:
    #     #             edge_index[0].append(u1)
    #     #             edge_index[1].append(u2)
    #     #     np.save('./datasets/Yelp/edge_index.npy', np.array(edge_index))
    #     #     edge_index = torch.tensor(edge_index)
    #     # edge_index = to_undirected(edge_index)
    all_nbr = data['train_nbr']
    all_nbr = torch.tensor(all_nbr)
    all_nbr = torch.cat((torch.zeros(1,args.nbr_maxlen).long(), all_nbr), dim=0)
    return train_loader, val_loader, test_loader, user_train, eval_users, all_nbr
...
def main():
    parser = argparse.ArgumentParser(description='meta_TEA-Gsage')
    parser.add_argument('--dataset', default='Yelp')
    parser.add_argument('--model', default='meta_TEA_Gsage')
    ...
    parser.add_argument('--res_file', type=str, default='gatGSAGEres.txt')
    parser.add_argument('--meta_gat_layer_num', type=int, default=1)
    ...
    print('Loading...')
    st = time()
    train_loader, val_loader, test_loader, user_train, eval_users, all_nbr = load_ds(args, item_num)
    ...
    for r in range(args.repeat):
        ...
        for epoch in range(1, args.max_epochs):
            st = time()
            train_loss = train(model, opt, lr_scheduler, train_loader, item_num, all_nbr, args)
            ...
            if epoch % args.check_epoch == 0 and epoch >= args.start_epoch:
                val_metrics = evaluate(model, val_loader, eval_users, all_nbr)
        ...
        test_metrics = evaluate(model, test_loader, eval_users, all_nbr)
    ...
    with open(args.res_file, 'a') as f:
        f.write(
            f'Yelp Gsage repeat{args.repeat} epoch{args.max_epochs} batch_size{args.batch_size} lr{args.lr} loss_type{args.loss_type} drop{args.droprate}\n')
