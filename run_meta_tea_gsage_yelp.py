...
class CRFGNN(nn.Module):
    def __init__(self,
        ...
        self.seq_lin = nn.Linear(edim + edim + edim + edim, edim)
        ...
        self.meta_rnn = nn.GRU(input_size=edim, hidden_size=edim, num_layers=1, batch_first=True)
...
def main():
    ...
    parser.add_argument('--res_file', type=str, default='GSAGEres.txt')
    ...
    with open(args.res_file, 'a') as f:
        f.write(f'Yelp Gsage repeat{args.repeat} epoch{args.max_epochs} batch_size{args.batch_size} lr{args.lr} loss_type{args.loss_type} drop{args.droprate}\n')
