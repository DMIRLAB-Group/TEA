...
class DGRec(nn.Module):
    ...
    def forward(self, batch):
        ...
        hu = self.dgrec_forward(uid, seq, nbr, nbr_iid)  # B x d

        # target item emb (1-seesion)
        #pos_hi = self.item_embs(pos[:, -1].to(self.dev))  # B x d
        #neg_hi = self.item_embs(neg[:, -1].to(self.dev))

        pos_hi = self.dropout(self.item_embs(pos.to(self.dev)))  # B x item_len x d
    ...
    def predict(self, eval_batch):
        ...
        with torch.no_grad():
            ...
            logits = self.pred(hu, hi).squeeze()
            return logits
